        ...
        for kwarg in kwargs:
            if kwarg not in tolerated_extra_json:
                ...
                if kwarg.endswith("_count"):
                    array_attr = kwarg[: -len("_count")]  # Remove "_count" to get the array attribute name
                    try:
                        possible_array = getattr(self._json_attrs, array_attr)
                        if not isinstance(possible_array, list):
                            raise CRIPTExtraJsonAttributes(self.node_type, kwarg)
                    except AttributeError:
                        # Attribute not found in JsonAttributes
                        raise CRIPTExtraJsonAttributes(self.node_type, kwarg)

                # Handle other attributes
                elif kwarg not in tolerated_extra_json:
                    try:
